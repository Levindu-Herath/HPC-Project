...
ThreadArg *global_thread_args = NULL; 

int num_points = 0, num_clusters = 0, dimensions = 0, num_threads = 4; 
...
void* worker_thread(void* arg) {
    ...
    while (1) {
        if (global_iteration >= global_max_iterations || global_total_changes == 0) break;

        t_arg->local_changes = 0;
        for (int k = 0; k < num_clusters; k++) {
            t_arg->local_centroid_counts[k] = 0;
            for (int d = 0; d < dimensions; d++) t_arg->local_centroid_coords[k][d] = 0.0;
        }
        ...
        for (int i = t_arg->start_idx; i < t_arg->end_idx; i++) {
            ...
            for (int k = 0; k < num_clusters; k++) {
                ...
                if (distance < min_distance) { min_distance = distance; nearest_cluster = k; }
            }
        ...
        if (id == 0) {
            ...
            if (global_iteration % 10 == 0 || global_total_changes == 0) {
                ...
                double rmse = sqrt(wcss / num_points);
                printf("[Iter %3d] WCSS: %15.6f | RMSE: %9.6f | Changes: %6d\n", 
                       global_iteration, wcss, rmse, global_total_changes);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    double final_wcss = calculate_wcss();
    double final_rmse = sqrt(final_wcss / num_points);

    printf("--------------------------------------------------\n");
    printf("Converged  : %s (%d iterations)\n", (global_total_changes == 0 ? "Yes" : "No"), global_iteration);
    ...
    for (int i = 0; i < num_threads; i++) {
        for (int k = 0; k < num_clusters; k++) free(global_thread_args[i].local_centroid_coords[k]);
